...
namespace cf {
    ...
    template <typename Ref> using managed = std::unique_ptr<std::remove_pointer_t<Ref>, __cf_deleter<Ref>>;
     ...
     * @throw std::bad_cast If the number cannot be converted into the
     *   requested type without loss.
     */
     ...
     * @param allow_lossy If true, allow conversions that would result
     *   in a loss of precision.
     * @return The value contained by the CoreFoundation number.
     * @throw @c std::bad_cast If the number cannot be converted into
     *   the requested type without loss and @p allow_lossy is @c
     *   false.
     */
